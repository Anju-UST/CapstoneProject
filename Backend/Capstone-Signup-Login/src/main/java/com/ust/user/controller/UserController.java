...
import com.ust.user.entity.DtoOfUser;
...
public class UserController {
	...
	public ResponseEntity<?> logIn(@RequestBody User user)
	{
		...
		if(user1!=null){
			ResponseEntity<DtoOfUser> response = null;
			...
			if(usertype.equals("user")){
				...
				if(b) {
					Map<User, String> tokenMap = new HashMap<User, String>();
					...
					tokenMap.put(user1, token);
//					tokenMap.put(user1, "User Successfully logged in");
					DtoOfUser dto=new DtoOfUser(user1,token);
					response = new ResponseEntity<DtoOfUser>(dto, HttpStatus.OK);
				}
				else{
					return new ResponseEntity<User>(HttpStatus.NOT_FOUND);
				}
